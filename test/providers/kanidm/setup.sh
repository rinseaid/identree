...
# Mounted read-only in the tools container so kanidm can verify the server cert.
...
# The kanidm-certs volume is mounted so the tools container can verify the
# server's TLS cert via KANIDM_CA_PATH=/certs/ca.crt.
ke() {
    docker run --rm \
        --network "${NETWORK}" \
...
printf '%s\n' "$IDM_ADMIN_PW" | \
    docker run --rm -i \
        ...
        -e KANIDM_CA_PATH="/certs/ca.crt" \
        "${TOOLS_IMAGE}" \
        kanidm login -D idm_admin -H https://kanidm:8443 || {
    echo "ERROR: kanidm login failed."
    echo "Try manually: docker run --rm -it --network ${NETWORK} -v ${CERTS_VOLUME}:/certs:ro -e KANIDM_CA_PATH=/certs/ca.crt ${TOOLS_IMAGE} kanidm login -D idm_admin -H https://kanidm:8443"
    exit 1
}
