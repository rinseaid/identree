#!/bin/bash
# validate.sh — verify the Authentik SAML test environment is working.
# Checks:
#   1. identree healthz is up
#   2. SAML SP metadata is served at /saml/metadata
#   3. /saml/login redirects to Authentik (302 to IdP SSO URL)
#   4. PAM challenge flow works (shared secret auth)
set -euo pipefail

IDENTREE_URL="${IDENTREE_URL:-http://localhost:8100}"
SHARED_SECRET="${SHARED_SECRET:-test-shared-secret-authentik-saml-1234}"
CLIENT_CONTAINER="${CLIENT_CONTAINER:-identree-authentik-saml-client}"
