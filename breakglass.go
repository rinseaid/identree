...
func authenticateBreakglass(username, hashFilePath string) error {
	...
	if err := checkBreakglassRateLimit(); err != nil {
		fmt.Fprintf(os.Stderr, "identree: BREAKGLASS: rate limited for user %q: %v\n", username, err)
		return err
	}
	...
	if err := bcrypt.CompareHashAndPassword([]byte(hash), password); err != nil {
		fmt.Fprintf(os.Stderr, "identree: BREAKGLASS: authentication FAILED for user %q\n", username)
		recordBreakglassFailure()
		fmt.Fprintf(tty, "  Authentication failed.\n\n")
		return fmt.Errorf("break-glass authentication failed")
	}

	fmt.Fprintf(os.Stderr, "identree: BREAKGLASS: authentication SUCCESS for user %q\n", username)
...
func isServerUnreachable(err error) bool {
	...
	// the backend (identree) is down even though the proxy (Traefik,
...
func rotateBreakglass(cfg *ClientConfig, force, quiet bool) (plaintext string, err error) {
...
func escrowPassword(cfg *ClientConfig, hostname, password string, quiet bool) error {
	if cfg.ServerURL == "" {
...
func maybeRotateBreakglass(cfg *ClientConfig, rotateBefore time.Time) {
	...
	if !breakglassFileExists(cfg.BreakglassFile) {
		...
		if _, err := rotateBreakglass(cfg, true, true); err != nil {
			fmt.Fprintf(os.Stderr, "identree: initial break-glass setup failed: %v\n", err)
		}
		return
	}
	...
	if _, err := rotateBreakglass(cfg, true, true); err != nil {
		fmt.Fprintf(os.Stderr, "identree: break-glass rotation failed: %v\n", err)
	}
}
