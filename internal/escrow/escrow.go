...
// Backend is the interface implemented by native escrow backends.
...
func NewEscrowBackend(cfg *config.ServerConfig) Backend {
...
//   - ESCROW_PATH        = KV mount + path prefix (e.g., "secret/identree")
//
...
func (b *hcVaultBackend) Store(ctx context.Context, hostname, password, vault string) (string, string, error) {
	...
	// Split path into mount + prefix. e.g., "secret/identree" → "secret", "identree"
	mount, prefix, hasPrefix := strings.Cut(path, "/")
	var kvPath string
	if hasPrefix {
		kvPath = fmt.Sprintf("/v1/%s/data/%s/%s", url.PathEscape(mount), url.PathEscape(prefix), url.PathEscape(hostname))
	} else {
		kvPath = fmt.Sprintf("/v1/%s/data/%s", url.PathEscape(mount), url.PathEscape(hostname))
	}
