...
const sharedCSS = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Shared helpers — replaces repeated inline style="..." patterns */
    .config-input-narrow { max-width: 120px; }
    .chevron-icon { margin-left: 1px; }
    .u-hidden { display: none; }
    .form-inline { display: inline; }
    .form-signout { display: inline; margin: 0; }
    .lock-icon { color: var(--text-3); flex-shrink: 0; }
    .hint-muted { font-size: 0.75rem; color: var(--text-3); }
    .reason-quote { font-size: 0.8125rem; color: var(--text-2); font-style: italic; }
    .sub-label { font-size: 0.75rem; font-weight: 600; color: var(--text-3); text-transform: uppercase; letter-spacing: 0.04em; margin-bottom: 8px; }
    .empty-state-centered { text-align: center; color: var(--text-2); font-size: 0.875rem; padding: 20px 0; }
    .signout-btn { width: 100%; text-align: left; background: none; border: none; cursor: pointer; color: var(--danger); font: inherit; font-size: 0.8125rem; font-weight: 500; padding: 7px 14px; }
    .reject-reason-input { font-size: 0.75rem; padding: 3px 7px; border: 1px solid var(--border); border-radius: 5px; background: var(--surface); color: var(--text); width: 120px; }
    /* Reveal breakglass modal */
    .reveal-modal-box { max-width: 520px; padding: 28px 28px 24px; }
    .reveal-modal-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px; }
    .reveal-modal-head #reveal-modal-title { font-weight: 600; font-size: 1rem; }
    .reveal-modal-x { background: none; border: none; cursor: pointer; color: var(--text-2); font-size: 1.3rem; line-height: 1; padding: 2px 6px; }
    .reveal-modal-host-line { font-size: 0.85rem; color: var(--text-2); margin-bottom: 14px; }
    .reveal-notice { background: var(--danger-bg); border: 1px solid var(--danger-border); color: var(--danger); border-radius: 8px; padding: 10px 14px; font-size: 0.8125rem; margin-bottom: 8px; }
    .reveal-rotate-note { background: rgba(217,119,6,0.1); border: 1px solid rgba(217,119,6,0.35); color: #d97706; border-radius: 8px; padding: 8px 14px; font-size: 0.8125rem; margin-bottom: 8px; }
    .reveal-pw-row { display: flex; align-items: center; gap: 10px; background: var(--bg); border: 1px solid var(--border); border-radius: 8px; padding: 10px 14px; font-family: monospace; font-size: 0.95rem; margin: 16px 0; }
    .reveal-pw-text { flex: 1; word-break: break-all; }
`
...
<div class="pending-bar" aria-live="polite" aria-atomic="false">
  ...
  <span class="pbar-main">
    {{if $.IsAdmin}}<strong class="pbar-host">{{.Username}}</strong><span class="pbar-sep">@</span>{{end}}<strong class="pbar-host">{{.Hostname}}</strong><span class="pbar-sep">·</span><span class="pbar-code">{{.Code}}</span><span class="pbar-sep">·</span><span class="pbar-exp">{{call $.T "expires_in"}} {{.ExpiresIn}}</span>{{if .Reason}}<span class="pbar-sep">·</span><span class="challenge-reason reason-quote">"{{.Reason}}"</span>{{end}}
  </span>
  <div class="pbar-actions">
    ...
    <form method="POST" action="/api/challenges/override" class="form-inline">
      <input type="hidden" name="challenge_id" value="{{.ID}}">
      <input type="hidden" name="username" value="{{$.Username}}">
      <input type="hidden" name="csrf_token" value="{{$.CSRFToken}}">
      <input type="hidden" name="csrf_ts" value="{{$.CSRFTs}}">
    ...
    <span class="reject-inline">
      ...
      <form method="POST" action="/api/challenges/reject" class="reject-inline-form" style="display:none">
        ...
        <input type="hidden" name="csrf_token" value="{{$.CSRFToken}}">
        <input type="hidden" name="csrf_ts" value="{{$.CSRFTs}}">
        <input type="text" name="deny_reason" maxlength="500" placeholder="{{call $.T "reason_optional"}}" class="reject-reason-input" style="width:160px">
  ...
  <div class="modal-box pending-modal-box">
      ...
      <div class="pending-table-row" role="row">
        ...
        <div class="gtcol" role="cell">
          ...
          <span class="just-pick" data-required="false">
            ...
            <span class="reason-quote">"{{.Reason}}"</span>
          </span>
          {{else}}
          <span class="just-pick" data-required="{{if $.RequireJustification}}true{{else}}false{{end}}">
            <select class="just-sel">
        ...
        <div class="gtcol pending-table-actions" role="cell">
          ...
          <span class="reject-inline">
            ...
            <span class="reject-inline-row" style="display:none">
              <input type="text" maxlength="500" placeholder="{{call $.T "reason_optional"}}" class="reject-reason-input">
    ...
    <div class="pending-table-footer" style="border-top:1px solid var(--border);display:flex;justify-content:flex-end;gap:6px;padding:12px 12px 10px">
      <div class="pending-table-actions" style="flex:none">
        <form method="POST" action="/api/challenges/approve-all" class="form-inline">
          <input type="hidden" name="username" value="{{.Username}}">
          <input type="hidden" name="csrf_token" value="{{.CSRFToken}}">
          <input type="hidden" name="csrf_ts" value="{{.CSRFTs}}">
        ...
        <form method="POST" action="/api/challenges/reject-all" class="form-inline">
          <input type="hidden" name="username" value="{{.Username}}">
          <input type="hidden" name="csrf_token" value="{{.CSRFToken}}">
          <input type="hidden" name="csrf_ts" value="{{.CSRFTs}}">
      ...
      <div class="sessions-table-row{{if and $.HighlightUser (eq .Username $.HighlightUser) (eq .Hostname $.HighlightHost)}} session-highlight{{end}}" role="row" data-user="{{.Username}}" data-host="{{.Hostname}}">
        ...
        <div class="gtcol gtcol-sactions" role="cell" style="gap:6px;flex-wrap:nowrap;align-items:center;">
          <div class="elevate-wrap">
            <button type="button" class="saction-btn saction-primary elevate-toggle"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>{{call $.T "extend"}}<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="chevron-icon"><polyline points="6 9 12 15 18 9"/></svg></button>
    ...
    <div class="sessions-table sessions-table--user" id="user-sessions-table" role="table" aria-label="{{call .T "sessions"}}">
      <div class="sessions-table-header" role="row">
        <div class="gtcol gtcol-shost" role="columnheader" style="gap:8px;align-items:center">
          <button type="button" class="filter-toggle-btn" id="user-sessions-filter-toggle" aria-label="Toggle filters"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg></button>
      ...
      <div class="sessions-table-row{{if and $.HighlightHost (eq .Hostname $.HighlightHost)}} session-highlight{{end}}" role="row" data-host="{{.Hostname}}">
        ...
        <div class="gtcol gtcol-sactions" role="cell" style="gap:6px;flex-wrap:nowrap;align-items:center;">
          ...
          <form method="POST" action="/api/sessions/revoke" class="form-inline">
            ...
            <input type="hidden" name="csrf_token" value="{{$.CSRFToken}}">
            <input type="hidden" name="csrf_ts" value="{{$.CSRFTs}}">
            <button type="submit" class="saction-btn saction-danger saction-confirm" data-confirm="{{printf (call $.T "confirm_revoke_session") .Hostname}}"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>{{call $.T "revoke"}}</button>
          </form>
        </div>
      </div>
      {{end}}{{end}}
    ...
    if(deployOpenBtn){
      ...
      deploySubmitBtn.addEventListener('click',function(){
        ...
        .then(function(data){
          ...
          var _deployDone=false;
          var _deployDelay=1000;
          var _deployRetries=0;
          var _deployMaxRetries=5;
          var es;
          var _deployUnload=function(){if(es)es.close();};
          ...
          function connectDeployStream(){
            ...
            es.addEventListener('message',function(e){
              _deployDelay=1000;_deployRetries=0;
            ...
            es.onerror=function(){
              es.close();
              if(_deployDone)return;
              _deployRetries++;
              if(_deployRetries>_deployMaxRetries){
                window.removeEventListener('beforeunload',_deployUnload);
                if(!statusEl.textContent){statusEl.textContent=_t.connLost;statusEl.className='deploy-status err';}
                return;
              }
              setTimeout(connectDeployStream,_deployDelay);
              _deployDelay=Math.min(_deployDelay*2,30000);
            };
          }
          connectDeployStream();
        })
  ...
  <main class="main" id="main-content">
    ...
    <div class="config-table" id="config-table">
      <div class="config-filter-header">
        <button type="button" class="filter-toggle-btn" id="config-filter-toggle" aria-label="Toggle filters"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg></button>
    ...
    <div class="config-table" style="margin-top:16px">
      ...
      <div style="padding:12px 16px;display:flex;flex-direction:column;gap:10px">
        ...
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          ...
          <label style="font-size:0.8125rem;display:flex;align-items:center;gap:4px"><input type="radio" name="install-mode" value="sssd"> PAM + SSSD{{if not .LDAPProvisionEnabled}} <span class="hint-muted">(requires IDENTREE_LDAP_PROVISION_ENABLED=true)</span>{{end}}</label>
        </div>
    ...
    <div class="users-table" id="users-table" role="table" aria-label="{{call .T "users"}}">
      <div class="users-table-header" role="row">
        <div class="gtcol gtcol-uname" role="columnheader" style="gap:8px;align-items:center">
          <button type="button" class="filter-toggle-btn" id="users-filter-toggle" aria-label="Toggle filters"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg></button>
      ...
      <div class="users-table-row" role="row">
        ...
        <div class="gtcol gtcol-uactions" role="cell" style="gap:6px;flex-wrap:nowrap;align-items:center;justify-content:flex-end;">
          {{if gt .ActiveSessions 0}}<a href="/?user={{.Username}}" class="saction-btn saction-sessions saction-primary"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>{{call $.T "sessions"}} ({{.ActiveSessions}})</a>{{end}}
          <a href="/access?user={{.Username}}" class="saction-btn"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg>{{call $.T "access"}}</a>
          <a href="/history?user={{.Username}}" class="saction-btn"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>{{call $.T "history"}}</a>
          <form method="POST" action="/api/users/remove" class="form-inline">
          ...
          {{if .UserID}}<button type="button" class="saction-btn ssh-keys-toggle" data-claims-target="uclaims-{{.UserID}}" data-user-id="{{.UserID}}"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>Claims <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="chevron-icon"><polyline points="6 9 12 15 18 9"/></svg></button>{{end}}
        </div>
      </div>
      {{if .UserID}}
      <div class="user-claims-panel" id="uclaims-{{.UserID}}" data-user-id="{{.UserID}}" data-username="{{.Username}}">
        ...
        <form method="POST" action="/api/admin/users/claims" class="ssh-keys-form">
          ...
          <div class="sub-label">SSH Public Keys</div>
          ...
          <div class="claims-readonly">
            <div class="sub-label" style="margin-bottom:6px">Read-only claims (not managed by identree)</div>
    ...
    <div class="groups-table" id="groups-table" role="table" aria-label="{{call .T "groups"}}">
      <div class="groups-table-header" role="row">
        <div class="gtcol gtcol-name" role="columnheader" style="gap:8px;align-items:center">
          <button type="button" class="filter-toggle-btn" id="groups-filter-toggle" aria-label="Toggle filters"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg></button>
    ...
    <div class="hosts-table" id="hosts-table" role="table" aria-label="{{call .T "hosts"}}">
      <div class="hosts-table-header" role="row">
        <div class="gtcol gtcol-hhost" role="columnheader" style="gap:10px;align-items:center;flex-wrap:wrap">
          <button type="button" class="filter-toggle-btn" id="hosts-filter-toggle" aria-label="Toggle filters"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg></button>
      ...
      <div class="hosts-table-row" role="row">
        <div class="gtcol gtcol-hhost" role="cell" style="flex-wrap:wrap;gap:4px;align-items:center">
          {{if .Reported}}<button type="button" class="host-expand-btn" aria-expanded="false" aria-label="Toggle agent details" data-host="{{.Hostname}}"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg></button>{{end}}
        ...
        <div class="gtcol gtcol-hactions" role="cell" style="gap:6px;flex-wrap:nowrap;align-items:center;justify-content:flex-end">
          ...
          <a href="/history?hostname={{.Hostname}}" class="saction-btn"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>{{call $.T "history"}}</a>
          <form method="POST" action="/api/hosts/rotate" class="form-inline">
            <input type="hidden" name="hostname" value="{{.Hostname}}">
            <input type="hidden" name="username" value="{{$.Username}}">
            <input type="hidden" name="csrf_token" value="{{$.CSRFToken}}">
            <input type="hidden" name="csrf_ts" value="{{$.CSRFTs}}">
            <button type="submit" class="saction-btn saction-rotate" data-confirm="{{printf (call $.T "confirm_rotate_host") .Hostname}}"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.5"/></svg>{{call $.T "rotate"}}</button>
          </form>
        ...
        btn.addEventListener('click',function(){
          ...
          fetch('/api/breakglass/reveal',{method:'POST',body:JSON.stringify({hostname:hostname}),headers:Object.assign({'Content-Type':'application/json'},_csrf)})
            ...
            .then(function(data){
              ...
              if(rotateNote)rotateNote.classList.add('u-hidden');
              var rotated=false;
              copyBtn.onclick=function(){
                ...
                if(!rotated){
                  ...
                  fetch('/api/hosts/rotate',{method:'POST',body:body,headers:{'Content-Type':'application/x-www-form-urlencoded'}})
                    .then(function(rr){if(rr.status===401){window.location.href='/login';return;}if(rr.ok&&rotateNote)rotateNote.classList.remove('u-hidden');})
                    .catch(function(){});
                }
              };
    ...
    <div id="reveal-modal" class="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="reveal-modal-title">
      <div class="modal-box reveal-modal-box">
        <div class="reveal-modal-head">
          <span id="reveal-modal-title">{{call .T "breakglass_password"}}</span>
          <button id="reveal-modal-x" type="button" class="reveal-modal-x" aria-label="{{call .T "close"}}">&times;</button>
        </div>
        <div class="reveal-modal-host-line">{{call .T "reveal_host_label"}}: <strong id="reveal-modal-host"></strong></div>
        <div class="reveal-notice">{{call .T "reveal_audit_notice"}}</div>
        <div id="reveal-modal-rotate-note" class="reveal-rotate-note u-hidden">{{call .T "reveal_rotation_notice"}}</div>
        <div class="reveal-pw-row">
          <span id="reveal-modal-password" class="reveal-pw-text"></span>
          <button id="reveal-toggle" type="button" class="btn btn-sm">{{call .T "show"}}</button>
          <button id="reveal-copy" type="button" class="btn btn-sm">{{call .T "copy"}}</button>
        </div>
        <div class="modal-actions"><button id="reveal-modal-close" type="button" class="btn">{{call .T "close"}}</button></div>
      </div>
    </div>
    {{else}}
      ...
      <div class="row" role="listitem" style="flex-direction:column;align-items:stretch;gap:0">
        <div class="host-row-header">
          ...
          <div class="host-row-header-actions">
            ...
            <form method="POST" action="/api/sudo-rules/delete" class="form-inline">
      ...
      <div class="row" role="listitem">
        <div class="host-row-header">
          <div class="host-row-header-info">
            ...
            {{if .URL}}<span class="pill hint-muted">{{.URL}}</span>{{end}}
          </div>
          <div class="host-row-header-actions">
            <form method="POST" action="/api/admin/test-channel" class="form-inline">
            ...
            <form method="POST" action="/api/notification/channels/delete" class="form-inline">
    ...
    <div class="list" role="list">
      ...
      <div class="row" role="listitem">
        <div class="host-row-header">
          ...
          <div class="host-row-header-actions">
            <form method="POST" action="/api/notification/routes/delete" class="form-inline">
      ...
      <table class="info-table" style="width:100%;font-size:0.8125rem">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <form method="POST" action="/api/policies/delete" class="form-inline">
                <input type="hidden" name="username" value="{{$.Username}}">
                <input type="hidden" name="csrf_token" value="{{$.CSRFToken}}">
                <input type="hidden" name="csrf_ts" value="{{$.CSRFTs}}">
                <input type="hidden" name="name" value="{{.Name}}">
      ...
      <form method="POST" action="/api/policies/add">
        ...
        <table class="info-table" style="width:100%;font-size:0.8125rem">
          ...
          <tr>
            ...
            {{if .HostGroups}}<p class="hint-muted" style="margin:2px 0">Available: {{range $i, $g := .HostGroups}}{{if $i}}, {{end}}{{$g}}{{end}}</p>{{end}}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            {{if .ChannelNames}}<p class="hint-muted" style="margin:2px 0">Available: {{range $i, $n := .ChannelNames}}{{if $i}}, {{end}}{{$n}}{{end}}</p>{{end}}</td>
          </tr>
        </table>
        <div class="modal-actions" style="margin-top:12px">
        ...
        <div class="modal-field">
          ...
          <div id="remove-key-empty">
            <div class="key-upload-row">
              ...
              <button type="button" class="key-action-btn" id="remove-key-upload-btn"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg><span>{{call .T "deploy_key_upload"}}</span></button>
        ...
        <div class="modal-field">
          ...
          <div id="deploy-key-empty">
            <div class="key-upload-row">
              ...
              <button type="button" class="key-action-btn" id="deploy-key-upload-btn"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg><span>{{call .T "deploy_key_upload"}}</span></button>
      ...
      <div class="pending-table-row" role="row">
        ...
        <div role="cell" style="text-align:right;display:flex;gap:6px;justify-content:flex-end">
          <form method="POST" action="/api/challenges/approve" class="saction-form" style="display:flex;align-items:center;gap:4px">
            ...
            <input type="hidden" name="csrf_ts" value="{{$.CSRFTs}}">
            <input type="hidden" name="challenge_id" value="{{.ID}}">
            <input type="text" name="reason" maxlength="500" placeholder="{{call $.T "reason_optional"}}" class="reject-reason-input">
          ...
          <span class="reject-inline">
            ...
            <form method="POST" action="/api/challenges/reject" class="saction-form reject-inline-form" style="display:none;align-items:center;gap:4px">
              ...
              <input type="hidden" name="csrf_ts" value="{{$.CSRFTs}}">
              <input type="hidden" name="challenge_id" value="{{.ID}}">
              <input type="text" name="deny_reason" maxlength="500" placeholder="{{call $.T "reason_optional"}}" class="reject-reason-input">
          ...
          <div class="access-host-row">
            ...
            <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;justify-content:flex-end">
              ...
              <form method="POST" action="/api/sessions/revoke" class="form-inline">
                ...
                <button type="submit" class="saction-btn saction-danger access-saction-confirm" data-confirm="{{printf (call $.T "confirm_revoke_session_user") .Username .Hostname}}"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>{{call $.T "revoke"}}</button>
              </form>
              <div class="elevate-wrap">
                <button type="button" class="saction-btn saction-primary elevate-toggle"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>{{call $.T "extend"}}<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="chevron-icon"><polyline points="6 9 12 15 18 9"/></svg></button>
                <form method="POST" action="/api/sessions/extend" class="elevate-menu">
              ...
              {{if not .Active}}
              <div class="elevate-wrap">
                <button type="button" class="saction-btn saction-primary elevate-toggle"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg>{{call $.T "elevate"}}<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="chevron-icon"><polyline points="6 9 12 15 18 9"/></svg></button>
