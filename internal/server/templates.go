...
<head>
  ...
  <style>` + sharedCSS + navCSS + `
    ...
    .just-me-btn { display: inline-flex; align-items: center; gap: 3px; padding: 1px 7px; border-radius: 10px; font-size: 0.6875rem; font-weight: 500; text-decoration: none; border: 1px solid var(--border); background: var(--surface); color: var(--text-2); white-space: nowrap; transition: background 0.12s, color 0.12s, border-color 0.12s; line-height: 1.6; }
    .just-me-btn:hover { background: var(--primary-sub); color: var(--primary); border-color: var(--primary); }
    .just-me-btn.active { background: var(--primary-sub); color: var(--primary); border-color: var(--primary); }
    .just-me-btn.active:hover { background: var(--danger-bg); color: var(--danger); border-color: var(--danger); }
  </style>
  <script nonce="{{.CSPNonce}}">
  if(!document.cookie.split(';').some(function(c){return c.trim().indexOf('pam_tz=')===0;})){
    var tz=Intl.DateTimeFormat().resolvedOptions().timeZone;
  ...
  <main class="main" id="main-content">
    ...
    <div class="history-gtable" id="history-gtable">
      <div class="history-gtable-header">
        ...
        {{if .IsAdmin}}<div class="gtcol gtcol-huser" style="gap:6px;align-items:center"><a href="/history?sort=user&order={{if eq .Sort "user"}}{{if eq .Order "asc"}}desc{{else}}asc{{end}}{{else}}asc{{end}}&action={{.ActionFilter}}&hostname={{.HostFilter}}&user={{.UserFilter}}&per_page={{.PerPage}}" class="col-sort-link{{if eq .Sort "user"}} active{{end}}">{{call .T "user"}}{{if eq .Sort "user"}} {{if eq .Order "asc"}}↑{{else}}↓{{end}}{{end}}</a>{{if eq .UserFilter .Username}}<a href="/history?action={{.ActionFilter}}&hostname={{.HostFilter}}&sort={{.Sort}}&order={{.Order}}&per_page={{.PerPage}}" class="just-me-btn active" title="Clear user filter">just me ✕</a>{{else}}<a href="/history?action={{.ActionFilter}}&hostname={{.HostFilter}}&user={{.Username}}&sort={{.Sort}}&order={{.Order}}&per_page={{.PerPage}}" class="just-me-btn" title="Show only my activity">just me</a>{{end}}</div>{{end}}
